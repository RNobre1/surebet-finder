...
export function aggregateValueBets(allValueBetsNested: any[][]): InternalValueBet[] {
  // Flatten array
  ...
  for (const bet of flattened) {
    ...
    // robust key:
    const uniqueKey = bet.id || `${bet.eventId}-${bet.bookmaker}-${bet.betSide}-${bet.market?.name}-${bet.expectedValue}`;

  ...
  for (const event of eventsById.values()) {
    ...
    for (const marketKey of marketsSet.values()) {
      ...
      if (sumProbabilities < 1.0) {
        // Profit margin is (1 - sum) or (1 / sum) - 1. 
        // Standard formula: Profit % = (1 - sumProb) / sumProb
        // Wait, standard arbitrage margin: 1 - sumProbabilities.
        // Let's use Margin = 1 - sumProbabilities (e.g., 0.05 for 5%)
        // Actually, profit = 100 / sumProbabilities - 100. Let's return the literal surplus %.

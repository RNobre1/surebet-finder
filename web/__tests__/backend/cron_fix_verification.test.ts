...
describe('Cron Sync Events and Cache Fix Verification (Odds-API.io v3)', () => {
  ...
  it('getCronState should use "key" and parse "value"', async () => {
    const mockDbRow = {
      key: 'surebets_queue',
      value: JSON.stringify({ events: ['e1'] }),
    }
    eqMock.mockReturnValue({
  ...
  it('cron_sync_events should use v3 API and correct domain and mapping', async () => {
    ...
    await syncEventsHandler({} as HandlerEvent, {} as HandlerContext)
    ...
    expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('sport='))
    expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('league='))
  })
})
